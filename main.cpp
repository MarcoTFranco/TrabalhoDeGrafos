...
private:
    ...
    void retiraVertices();                                                                        // Função que retira os vértices de uma string
    void retiraArestas();                                                                         // Função que retira as arestas de uma string
    ...
    void dfsArvore(int v, vector<bool> &visitado, vector<int> &pais);                             // Função auxiliar DFS para construir e imprimir a árvore de profundidade
    void bfs(int v, vector<bool> &visitado);                                                      // Função que faz uma busca em largura
    void bfsArvore(int v, vector<bool> &visitado, vector<int> &pais);                             // Função auxiliar BFS para construir a árvore de largura
...
/**
 * Função que imprime a árvore de profundidade utilizando DFS
 */
void Grafo::imprimirArvoreProfundidade()
{
    vector<bool> visitado(vertices.size(), false);
    vector<int> pais(vertices.size(), -1); // Vetor para armazenar o pai de cada vértice na árvore de profundidade

    for (int i = 0; i < (int) vertices.size(); ++i)
    {
        if (!visitado[i])
        {
            dfsArvore(i, visitado, pais);
        }
    }

    // Imprime a árvore de profundidade
    cout << "Arvore de Profundidade (Vertice Pai -> Vertice Filho):" << endl;
    for (int i = 0; i < (int) vertices.size(); ++i)
    {
        if (pais[i] != -1)
        { // Se o vértice tem pai
            cout << pais[i] + 1 << " -> " << i + 1 << endl;
        }
    }
}

/**
 * Função auxiliar DFS para construir e imprimir a árvore de profundidade
 */
void Grafo::dfsArvore(int v, vector<bool> &visitado, vector<int> &pais)
{
    visitado[v] = true;

    for (int u : listaDeAdjacencia[v])
    {
        if (!visitado[u - 1])
        {
            pais[u - 1] = v; // Define v como pai de u na árvore de profundidade
            dfsArvore(u - 1, visitado, pais);
        }
    }
}
...
/**
 * Função auxiliar BFS para construir a árvore de largura
 */
void Grafo::bfsArvore(int v, vector<bool> &visitado, vector<int> &pais)
{
    queue<int> fila;
    fila.push(v);
    visitado[v - 1] = true; // Ajuste no índice de acesso a `visitado` para v - 1
    ...
    while (!fila.empty())
    {
        ...
        for (int u : listaDeAdjacencia[vertice - 1]) // Ajuste no índice de acesso a listaDeAdjacencia para vertice - 1
        {
            if (!visitado[u - 1])
            {
                visitado[u - 1] = true;
                pais[u - 1] = vertice - 1; // Define vertice - 1 como pai de u - 1
                fila.push(u);
            }
        }
    }
...
/**
 * Função que imprime a árvore de largura utilizando BFS
 */
void Grafo::imprimirArvoreLargura()
{
    vector<bool> visitado(vertices.size(), false);
    vector<int> pais(vertices.size(), -1); // Vetor para armazenar o pai de cada vértice na árvore de largura

    for (int i = 0; i < (int) vertices.size(); ++i)
    {
        if (!visitado[i])
        {
            bfsArvore(vertices[i], visitado, pais);
        }
    }

    // Imprime a árvore de largura (Vertice Pai -> Vertice Filho)
    cout << "Arvore de Largura (Vertice Pai -> Vertice Filho):" << endl;
    for (int i = 0; i < (int) vertices.size(); ++i)
    {
        if (pais[i] != -1) // Se o vértice tem pai
        {
            cout << pais[i] + 1 << " -> " << i + 1 << endl;
        }
    }
...
/**
 * Função que verifica se existe um ciclo hamiltoniano
 */
